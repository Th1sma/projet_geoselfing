...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    // Séléctionne uniquement les identifiants avec une requête SELECT 
    foreach ($tables as $table) {

        // Initialise un tableau assiociatif
        $data = [];
        $data['photo'] = [];
        $result = $handle->query("SELECT * FROM photo");
        ...
        while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
            ...
            $extract_photo = [
                "id" => $id,
                "img" => $img,
                "photo_fk_groupe" => $photo_fk_groupe
            ];

            $data['photo'][] = $extract_photo;
        }
